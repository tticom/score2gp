...
OBSERVATIONS_SCHEMA_VERSION: Literal["document_observations.v0.1"] = "document_observations.v0.1"
TOPOLOGY_SCHEMA_VERSION: Literal["document_topology.v0.1"] = "document_topology.v0.1"
GRAPH_SCHEMA_VERSION: Literal["recognition_graph.v0.1"] = "recognition_graph.v0.1"
RESOLUTION_SCHEMA_VERSION: Literal["resolution_result.v0.1"] = "resolution_result.v0.1"
MUSICAL_DOCUMENT_SCHEMA_VERSION: Literal["musical_document.v0.1"] = "musical_document.v0.1"
...
class DocumentObservations(BaseModel):
    ...
    def observation_ids_are_unique(self) -> "DocumentObservations":
        ...
        for v in self.vectors:
            if v.id in ids:
                raise ValueError(f"Duplicate observation id '{v.id}' in vector observations")
            ids.add(v.id)
        for t in self.texts:
            if t.id in ids:
                raise ValueError(f"Duplicate observation id '{t.id}' in text observations")
            ids.add(t.id)
        for r in self.rasters:
            if r.id in ids:
                raise ValueError(f"Duplicate observation id '{r.id}' in raster observations")
            ids.add(r.id)
