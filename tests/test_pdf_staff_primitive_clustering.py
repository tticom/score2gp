from score2gp.pdf_staff_notation_diagnostics import cluster_x_aligned_primitives, _Primitive
...
def test_primitives_within_half_staff_space_group_into_one_cluster() -> None:
    prims = [
        _Primitive("line", 10.0, 100.0, 10.0, 110.0),
        _Primitive("rect", 14.0, 105.0, 16.0, 107.0),
    ]
...
def test_primitives_farther_than_half_staff_space_form_separate_clusters() -> None:
    prims = [
        _Primitive("line", 10.0, 100.0, 10.0, 110.0),
        _Primitive("rect", 25.0, 105.0, 27.0, 107.0),
    ]
    clusters = cluster_x_aligned_primitives(prims, 10.0)
...
def test_cluster_ordering_is_deterministic_left_to_right() -> None:
    prims = [
        _Primitive("line", 50.0, 100.0, 50.0, 110.0),
        _Primitive("rect", 10.0, 105.0, 12.0, 107.0),
        _Primitive("curve", 30.0, 105.0, 32.0, 107.0),
    ]
    clusters = cluster_x_aligned_primitives(prims, 10.0)
...
def test_aggregate_counts_are_deterministic_and_correct() -> None:
    ...
    # max_cluster_vertical_span_staff_spaces
    # staff_space = 2.5
    # First cluster: line is y: 100 to 110, rect is 105 to 107. span = 10. 10/2.5 = 4.0
    # Second cluster: line is y: 100 to 110, span = 10. 10/2.5 = 4.0
    assert clustering.max_cluster_vertical_span_staff_spaces == 4.0
    
    # vertical strokes: 
    # line 1: dx=0, dy=10 >= 5.0 -> vertical stroke!
    # line 2: dx=0, dy=10 >= 5.0 -> vertical stroke!
    assert clustering.clusters_with_vertical_stroke_candidate == 2
...
def test_import_boundary_guardrail() -> None:
    ...
    # Search python files in src/score2gp
    ...
    # We shouldn't find these imports in ScoreIR, timing, TAB, playback files
    check_areas = [
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    for forbidden in forbidden_modules:
                        assert forbidden not in alias.name, f"{area} imports {forbidden}"
            elif isinstance(node, ast.ImportFrom):
                if node.module:
                    for forbidden in forbidden_modules:
                        assert forbidden not in node.module, f"{area} imports from {forbidden}"
