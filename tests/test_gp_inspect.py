...
def test_compare_gp_preserves_instrument_variant_track_names(tmp_path) -> None:
    gpif_acoustic = b"""<?xml version="1.0" encoding="utf-8"?>
<GPIF>
  <Score>
    <Tracks>
      <Track id="0">
...
"""
    gpif_electric = b"""<?xml version="1.0" encoding="utf-8"?>
<GPIF>
  <Score>
    <Tracks>
      <Track id="0">
...
"""
    path_acoustic = _create_synthetic_gp_package(tmp_path, "acoustic.gp", gpif_acoustic)
    path_electric = _create_synthetic_gp_package(tmp_path, "electric.gp", gpif_electric)

    comp = compare_gp(path_acoustic, path_electric)
    assert comp["matches"] is False
    assert comp["differences"]["tracks"] == {
        "expected": ["Acoustic Guitar"],
