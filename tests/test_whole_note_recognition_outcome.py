...
def test_half_note_read_only_recognition_outcome():
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent / "scripts"))
    from raster_diagnostics_gate_report import run_diagnostics_on_file
    from score2gp.whole_note_recogniser import map_half_note_candidates_to_read_only_outcomes

    pdf_path = Path(__file__).parent / "fixtures" / "pdf" / "generated_standard_staff_half_note.pdf"
    res = run_diagnostics_on_file(pdf_path)

    assert res is not None
    locations = res.get("half_note_candidate_locations", [])

    assert len(locations) == 2

    outcomes = map_half_note_candidates_to_read_only_outcomes(locations)

    assert len(outcomes) == 2
    assert outcomes[0]["symbol_type"] == "half_note_candidate"
    assert outcomes[0]["candidate_id"] == "half_note_candidate_001"
    assert "bbox" in outcomes[0]
    assert outcomes[0]["page_index"] == 1
    assert outcomes[0]["source"] == "diagnostic_candidate_evidence"
