...
- **Edge System Boundary Fallback Limits**: While a missing edge boundary can be inferred in highly constrained, safe cases, this fallback is strictly limited and conservative:
  - Fallback is strictly rejected if any rejected barlines exist in the inference direction (`pdf_bar_box_edge_boundary_ambiguous`), preventing automatic recovery in ambiguous systems.
  - Fallback is strictly rejected if the inferred box width would be less than 30.0pt (`pdf_bar_box_inferred_boundary_too_narrow`) or if a playable candidate lies within the ambiguous edge margin (`pdf_bar_box_inferred_boundary_candidate_ambiguous`).
  - Fallback boundaries are strictly marked as inferred with distinct provenance/warning telemetry; they are never accepted silently as observed barlines.
  - Fallback is only for clear system outer edges; missing *internal* (inter-measure) boundaries remain strictly unsafe and are never inferred.
  - Playable fret candidates still require safe system, bar, and string evidence; any unassigned candidate will block build-ir.
