...
- These preflight timing fixtures are tiny, synthetic, public reproductions of timing failure classes, focusing on compound meters (e.g., 12/8), voice cursor alignment, repeated backup/forward cursor movements, rest/note overlaps, and Audiveris-style heavy rewinds.
- Compound meter (e.g., 12/8), voice cursor reset, and backup/forward-heavy handling remain strictly conservative. Risky, overlapping, or unbalanced MusicXML timing strictly blocks alignment/build-ir.
