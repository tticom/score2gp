...
ASCII-tab PDFs are a separate input class from drawn tab staff geometry. Some born-digital PDFs contain text rows such as string labels followed by a pipe and ASCII tab characters rather than vector string lines. `extract-tab` detects these with `ascii-tab.v0.1`, groups nearby six-row blocks, assigns string numbers by row order, extracts fret numbers from character spans, and preserves inline markers such as slides, hammer-ons, pull-offs, bends, releases, and vibrato as non-playable technique-text evidence. ASCII grouping is diagnostic evidence only: without a timing source and safe bar/onset alignment, `build-ir` refuses the input with `ascii_tab_timing_unavailable` or `partial_ascii_tab_grouping` instead of guessing rhythm.
