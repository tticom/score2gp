...
When extraction finds playable-looking fret text but grouping is missing or partial, `extract-tab` writes `grouping-diagnostics.html` and one overlay image per page with candidate boxes. Open the HTML report first. It records that extraction succeeded, grouping failed, alignment was not attempted, and ScoreIR was not written. The overlays show the extracted tokens without pretending they are musically aligned.
