...
These fixtures are generated by `tests/fixtures/pdf/make_generated_ascii_tab_pdfs.py`. They model born-digital text PDFs where tablature is encoded as ASCII rows rather than drawn staff geometry. `extract-tab` uses `ascii-tab.v0.1` to group six-row ASCII blocks, assign row-order strings, extract fret tokens, estimate token x-position from character spans, and preserve inline technique markers as non-playable evidence. Complete ASCII grouping still emits `ascii_tab_timing_unavailable` because ASCII character positions are not musical timing. Malformed fewer-than-six-row blocks emit `partial_ascii_tab_grouping`. In both cases `build-ir` refuses ScoreIR output unless a future phase supplies safe timing and alignment.
