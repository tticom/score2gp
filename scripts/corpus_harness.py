...
def run_pipeline_for_input(
    ...
    cmd.extend(["--allow-remediation", "--allow-skip-unboxed-systems"])
    ...
    cli_executable_path = shutil.which("score2gp") or sys.executable
    python_import_path = str(Path(score2gp.__file__).resolve())
    classification = "pdf-tab-musicxml" if musicxml_path else "pdf-tab-only"

    structural_counts = {
        "bars": summary_counts.get("bar_count", 0),
        "events": summary_counts.get("event_count", 0),
        "source_rests": 0,
        "emitted_rests": 0,
        "timing_issues": summary_counts.get("warning_count", 0),
    }
